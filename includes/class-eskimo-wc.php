...
final class Eskimo_EPOS_WC { 
	...
	public function customer_created( $cust_id ) {
		error_log( __CLASS__ . ':' . __METHOD__ . ' Customer ID[' . $cust_id . ']' );
		...
		if ( $user_role === 'customer' ) {
			...
			$data = $response['body'];
			if ( $data === true ) {
				return error_log( 'Customer ID [' . $cust_id . '] Email[' . $user_email . '] Exists' );
			} else {
				$data = json_decode( $response['body'] );
				error_log( 'Customer EPOS [' . $data->params . '] Result[' . $data->result . ']' );
			}
			...
			error_log( 'EPOS Customer:  Route[' . $data['route'] . '] Params[' . $data['params'] . '] Result[' . $data['result'] . ']' );
		}
	}
	...
	public function new_order_created( $order_id ) {
		error_log( __CLASS__ . ':' . __METHOD__ . ' Order ID[' . $order_id . ']' );
		...
		if ( false === $user ) {
			return error_log( 'No user for this order[' . $order_id . ']' );
		}

		// OK, get user ID
		$user_id = $order->get_user_id();
		error_log( 'New Order ID[' . $order_id . '] Status[' . $order->get_status() . '] User[' . $user_id . ']' );
		...
		error_log( 'EPOS Order: Route[' . $data['route'] . '] Params[' . $data['params'] . '] Result[' . $data['result'] . ']' );
	}
	...
	public function order_status_processing( $order_id ) {
        error_log( __CLASS__ . ':' . __METHOD__ . ' ID[' . $order_id . ']' );
		...
		error_log( 'order_status_processing id[' . $order_id . '] user[' . $user_id . ']items[' . count( $order_items ) . ']status[' . $order->get_status() . ']' );
	...
	public function order_status_completed( $order_id ) {
		...
		error_log( 'order_status_completed id[' . $order_id . '] user[' . $user_id . ']items[' . count( $order_items ) . ']status[' . $order->get_status() . ']' );
