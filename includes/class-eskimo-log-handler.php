<?php
/**
 * Class Eskimo_Log_Handler_File file. Cloned from Woocommerce file handler.
 */
...
// Include log handler parent
require_once 'interface-eskimo-log-handler.php';

/**
 * Handles log entries by writing to a file.
 */
class Eskimo_Log_Handler implements Eskimo_Log_Handler_Interface {
     ...
     *
     * @param int $log_size_limit Optional. Size limit for log files. Default 8mb.
     */
	public function __construct( $log_size_limit = null ) {

		// Set default log size limit
        if ( null === $log_size_limit ) {
            $log_size_limit = 8 * 1024 * 1024;
        }
        $this->log_size_limit = apply_filters( 'eskimo_log_file_size_limit', $log_size_limit );
    ...
    public function handle( $timestamp, $message, $level ) {
		...
		// Set log contexts
		$levels 		= apply_filters( 'eskimo_logger_levels', $this->levels );
		$default_level 	= apply_filters( 'eskimo_logger_default_level', '' );

		// Set default log handle
		$handle = apply_filters( 'eskimo_logger_handle', 'eskimo' );

		// Set default level
		$level = ( empty( $level ) || ! in_array ( $level, $levels ) ) ? sanitize_text_field( $default_level ) : $level;

   		// Format entry
        $entry 	= self::format_entry( $timestamp, $level, $message );
     ...
     */
    protected static function format_entry( $timestamp, $level, $message ) {

        $time_string 	= self::format_time( $timestamp );
		$level_string 	= strtoupper( $level );
        $entry = ( empty( $level_string ) ) ? "{$time_string} {$message}" : "{$time_string} [{$level_string}] {$message}";

        return apply_filters( 'eskimo_format_log_entry', $entry, [
            'timestamp' => $timestamp,
            'level' 	=> $level,
            'message' 	=> $message
		] );
    ...
    public function clear( $handle ) {
        ...
        do_action( 'eskimo_log_clear', $handle );
    ...
    public function remove( $handle ) {
        ...
        if ( isset( $logs[ $handle ] ) && $logs[ $handle ] ) {
            $file = realpath( trailingslashit( ESKIMO_LOG_DIR ) . $logs[ $handle ] );
            if ( 0 === stripos( $file, realpath( trailingslashit( ESKIMO_LOG_DIR ) ) ) && is_file( $file ) && is_writable( $file ) ) { // phpcs:ignore WordPress.VIP.FileSystemWritesDisallow.file_ops_is_writable
                $this->close( $file ); // Close first to be certain no processes keep it alive after it is unlinked.
                $removed = unlink( $file ); // phpcs:ignore WordPress.VIP.FileSystemWritesDisallow.file_ops_unlink
            }
            do_action( 'eskimo_log_remove', $handle, $removed );
        }
        return $removed;
    }
	...
	public static function get_log_file_path( $handle ) {
		return trailingslashit( ESKIMO_LOG_DIR ) . self::get_log_file_name( $handle );
	}
	...
	public static function get_log_file_name( $handle ) {
		...
		return sanitize_file_name( implode( '-', [ $handle, $date_suffix ] ) . '.log' );
    }
    ...
    public static function delete_logs_before_timestamp( $timestamp = 0 ) {
        ...
        foreach ( $log_files as $log_file ) {
            $last_modified = filemtime( trailingslashit( ESKIMO_LOG_DIR ) . $log_file );

            if ( $last_modified < $timestamp ) {
                @unlink( trailingslashit( ESKIMO_LOG_DIR ) . $log_file ); // @codingStandardsIgnoreLine.
            }
        }
    }
    ...
    public static function get_log_files() {
        $files  = @scandir( ESKIMO_LOG_DIR ); // @codingStandardsIgnoreLine.
        $result = [];
